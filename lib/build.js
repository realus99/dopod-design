...
const REFERENCES = [
  ...
  },
  {
    file: 'figma.md',
    slug: 'figma',
    description: 'Carbon Figma variables and DTCG tokens, mapping a design value to a code token, and what to do with an off-scale value.',
    // Token files and lint config, never source. A designer handing over values
    // is a conversation, not an edit to a .tsx — attaching this to components
    // would surface it constantly and be useful almost never.
    globs: '**/*{token,tokens}*.{json,json5}',
  },
  {
    file: 'example.md',
    slug: 'example',
    description: 'A complete annotated build: UI Shell, 2x Grid, metric tiles, DataTable, and a validated form.',
    globs: '**/*.{tsx,jsx}',
  },
  {
    file: 'ibm-products.md',
    slug: 'ibm-products',
    description: 'Carbon for IBM Products (@carbon/ibm-products): Tearsheet, Datagrid, PageHeader, create flows, and the canary flag system.',
    globs: '**/*.{tsx,jsx,ts,js}',
  },
  {
    file: 'charts.md',
...
async function emitClaudeCode({ raw, body, references }, distDir) {
  ...
  const alwaysOn = [slim, referencePointer(location)].join('\n');
  await atomicWrite(path.join(target, 'AGENTS.md'), alwaysOn);

  // User scope reads ~/.cursor/rules/, not a home AGENTS.md — and a .mdc with
  // no frontmatter is "Apply Manually", which is worse than not shipping it.
  // So the same body goes out again carrying the marker that makes it always-on.
  await atomicWrite(
    path.join(target, 'user', `${PKG_NAME}-always.mdc`),
    renderFrontmatter({ alwaysApply: true }) + alwaysOn
  );
}
...
async function emitCursor({ data, body, references }, distDir) {
  ...
  // Cursor had no always-on layer at all (#32). `alwaysApply: false` with a
  // description is Cursor's "Apply Intelligently" — the agent picks the rule
  // from its description, which is precisely the failure the always-on layer
  // exists to prevent: 6 of 40 trigger-eval queries fail under every wording we
  // could write. Every other tool gets a file its host reads unconditionally.
  //
  // Cursor now reads AGENTS.md as a documented alternative to .cursor/rules, so
  // it joins claude-code and codex on the file they already share rather than
  // gaining a fourth .cursor artefact. install dedupes the common target.
  const location = '.cursor/rules/dopod-design-';
  let slim = extractSlim(body);
  for (const ref of references) {
    slim = slim.replaceAll(`\`references/${ref.file}\``, `\`${location}${ref.slug}.mdc\``);
  }
  const alwaysOn = [slim, referencePointer(location)].join('\n');
  await atomicWrite(path.join(target, 'AGENTS.md'), alwaysOn);
...
async function emitCopilot({ body, references }, distDir) {
  ...
  // User scope reads ~/.copilot/instructions, which only loads *.instructions.md
  // — there is no user-level equivalent of copilot-instructions.md. So the slim
  // always-on body needs its own file here, and it needs `applyTo` to apply at
  // all: without frontmatter VS Code loads the file but never attaches it
  // automatically, which would install cleanly and then do nothing.
  await atomicWrite(
    path.join(distDir, 'copilot', 'user', 'dopod-design.instructions.md'),
    renderFrontmatter({ applyTo: '**' }) +
      [slim, referencePointer('~/.copilot/instructions/')].join('\n')
  );
}
...
// Emitted once and installed when any pointer tool is selected.
async function emitSharedReferences({ references }, distDir) {
  for (const ref of references) {
    await atomicWrite(
...
// Windsurf caps a workspace rule at 12,000 characters. Four references exceed
// it, so only those that fit ship as rules; the rest are reachable through the
// pointer, which every tool gets.
const WINDSURF_CHAR_CAP = 12000;

async function emitWindsurf({ body, references }, distDir) {
  const target = path.join(distDir, 'windsurf');
  const location = '.dopod-design/references/';

  // trigger: always_on is the equivalent of AGENTS.md — no frontmatter dance.
  await atomicWrite(
    path.join(target, 'rules', `${PKG_NAME}.md`),
    renderFrontmatter({ trigger: 'always_on' })
      + relocateReferenceLinks(extractSlim(body), location)
      + referencePointer(location)
  );

  for (const ref of references) {
    if (ref.content.length > WINDSURF_CHAR_CAP) continue;   // pointer covers it
    const fm = ref.manual
      ? { trigger: 'manual', description: ref.description }
      : { trigger: 'glob', globs: ref.globs, description: ref.description };
    await atomicWrite(
      path.join(target, 'rules', `${PKG_NAME}-${ref.slug}.md`),
      renderFrontmatter(fm) + ref.content
    );
  }
}

async function emitCline({ body, references }, distDir) {
  const target = path.join(distDir, 'cline');
  const location = '.dopod-design/references/';

  // Cline merges every file in .clinerules/ into one always-on rule set, in
  // alphanumeric order. Shipping twelve references there would put the whole
  // skill in every prompt, so only the slim body goes in; the rest is pointed
  // at. The numeric prefix is the community convention for ordering.
  await atomicWrite(
    path.join(target, `10-${PKG_NAME}.md`),
    relocateReferenceLinks(extractSlim(body), location) + referencePointer(location)
  );
}

async function emitGemini({ body, references }, distDir) {
  const target = path.join(distDir, 'gemini');
  const location = '.dopod-design/references/';

  // GEMINI.md files are concatenated hierarchically and sent with every prompt,
  // so this is an always-on file and must stay slim. It may already exist and
  // belong to the user, so it is marker-merged.
  await atomicWrite(
    path.join(target, 'GEMINI.md'),
    [relocateReferenceLinks(extractSlim(body), location), referencePointer(location)].join('\n')
  );
}
...
async function build({ sourceDir, distDir }) {
  ...
  await emitWindsurf(canonical, distDir);
  await emitCline(canonical, distDir);
  await emitGemini(canonical, distDir);
  await emitSharedReferences(canonical, distDir);
  ...
  // Identity of the emitted payload, as distinct from canonical_sha256 which
  // covers SKILL.md alone and so misses every reference change. `check` uses
  // this to tell "you edited this file" from "a different build wrote it" —
  // the two need different advice and used to read identically.
  // Files are already sorted by path above, so this is stable across machines.
  const payloadDigest = sha256(
    files.map((f) => `${f.path}\0${f.sha256}\n`).join('')
  );
