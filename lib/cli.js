...
Flags
  ...
  --global         Install at user scope (~/) instead of this project.
                   Windsurf and Cline are skipped here, with a note.
...
Where files land (project scope)
  ...
  size-capped or outside the home dotfile convention. Those tools are skipped,
  with a note, rather than silently.

  Copilot's user scope is ~/.copilot/instructions, which loads only
  *.instructions.md — so copilot-instructions.md has no user-level equivalent
  and its content ships as dopod-design.instructions.md instead.
