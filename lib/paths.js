...
function resolveTargetPath(base, distRelPath, { global: isGlobal = false } = {}) {
  ...
  switch (tool) {
    ...
    case 'cursor':
      // Cursor's always-on layer (#32). At project scope this is the same
      // AGENTS.md claude-code and codex write, and install merges one copy.
      // At user scope Cursor reads ~/.cursor/rules/, not a home AGENTS.md, so
      // the always-on rule goes there instead of somewhere it would be ignored.
      if (rest === 'AGENTS.md') {
        // Project scope only. Cursor reads AGENTS.md in a workspace, but at
        // user scope it reads ~/.cursor/rules/ — see the user/ variant below,
        // which carries the alwaysApply marker a bare .md cannot.
        return isGlobal ? null : path.join(base, 'AGENTS.md');
      }
      if (rest.startsWith('user/')) {
        return isGlobal
          ? path.join(base, '.cursor', 'rules', path.basename(rest))
          : null;
      }
